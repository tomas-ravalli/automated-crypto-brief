...
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY') # [MODIFIED] Removed unnecessary GEMINI_API_SECRET
...
# [NEW] Function to get news summary from Gemini API
def get_news_summary(api_key, crypto_name):
    ...
    prompt = f"In one short paragraph, what is the most relevant news or milestone for {crypto_name} (e.g., Ripple's XRP) in the last 72 hours? Be concise."
...
# Function signature updated to accept news_summary
def send_email(current_price, avg_purchase_price, chart_path, news_summary):
    ...
    # Plain text body updated with news
    body_top = f"""
    ...
    # HTML body updated with news
    if chart_path:
        image_cid = make_msgid(domain='t-ravalli-report')[1:-1]
        
        html_body = f"""
        <html><body>
            <pre style="font-family: sans-serif; margin: 0;">{body_top}</pre>
            <hr>
            <h3 style="font-family: sans-serif; margin-bottom: 5px;">Latest News</h3>
            ...
            <pre style="font-family: sans-serif; margin: 0;">{body_bottom}</pre>
        </body></html>"""
...
if __name__ == "__main__":
    ...
    if avg_price is not None:
        ...
        if current_price is not None:
            ...
            # Get crypto name and fetch news summary
            ...
            # Pass news_summary to the send_email function
